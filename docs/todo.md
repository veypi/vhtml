...
- [x] **P3-2 defineRegistry 不随 clear() 清空**：clear()（module.js:214-220；loader reset 路径经此）触发后 `__vhtml_dev.defines` 积累 stale 条目。**已落地（2026-08-28）**：manager.clear() 增 `defineRegistry.length = 0`（原数组引用，__vhtml_dev.defines 同步清空）。
...
**状态（2026-08-29 落定）**：AOT 撤回与 vmessage 迁入 vhtml-ui 已落地，4000 全页面回归通过（用户验证）；SKILL.md/镜像/README/CHANGELOG 终稿随本提交冻结，semver 纪律自本版本起生效——此后破坏性改动只走大版本。

- [x] **AOT 绑定计划评估 —— 已测量并落地 → 收益不成立，已撤回（2026-08-29）**：
  **测量（bench/aot.mjs，happy-dom + 真实语料 messages.html 行模板）**：结构编译（compileNode 累计）占挂载耗时 62-92%，据此按方案 ② 落地 v-for 行模板计划化（buildRowPlan 静态解析一次 → instantiateRowPlan 按计划执行绑定安装），169/169 测试绿，4000 回归通过。
  **撤回理由（2026-08-29 bench 复跑 + 代码结构复核定论）**：① 收益对象错误——「结构编译」大头是 compileAttrs 绑定安装 / 插值 watch / DOM 操作，计划化只省 compileNode 入口分发（纳秒级）；实测 full 真实消息行 200 行 plan 63.5ms vs 旧 57.9ms（持平偏负，噪声域内），仅 plain 纯结构行 -40%（真实页面无此场景）。② 结构性债务不成立——instantiateRowPlan 是 compileVif.flushChain / compileNodeInner 的逐行镜像，双路径永久共存（回退不可删），落地期间已产生 3 个镜像漂移真 bug（boundary 误挂每层 / 组件 nodeName 大小写 / v-if 链跨文本吞插值）；row-plan ↔ compiler 循环 import 靠调用期绑定维持；诊断计时口径嵌套重叠（planMs 25.1 < elMs 9.8 + ifMs 27.0）。
  **撤回清单**：删 src/row-plan.js、compileVfor 计划分支与 window.__VHTML_NO_PLAN、compile-stats plan*Ms 五项及计时器、test/row-plan.test.js、bench/aot.mjs；SKILL.md Compile stats 段同步收窄。**保留**：P4 的 compileVfor disposeRuntimeSubtree(dom)（旧路径修复，与计划化无关）；compile-stats 的 nodeCompiles/nodeMs/codeCompiles/codeMs/vforLines（轻量诊断，__vhtml_dev 观测层先例）。
  **教训存档**：编译优化立项前必须把「分发开销」与「绑定安装/DOM」在计时上拆开测量——本轮 62-92% 占比把两者混在一起，导致优化入口找错。若将来重做行实例化优化，入口在绑定安装层（compileAttrs 属性级计划 / watch 合并），需新的测量支撑。
- [x] **vmessage 移出框架内核 —— 成本已核实，建议做（2026-08-28 综合评估 + 用户定装配方式）**：vmessage.js 零内核 import（纯自包含，544 行硬编码样式/位置），唯一挂载点 module.js:87-88（`sys.$message = vmessage`）。**装配方式（2026-08-28 用户定）：挪入 vhtml-ui，经 vhtml-ui 的 env.js `all.define('$message', vmessage)` 定义到 manager.globals——aic/vbase 全部调用面零改动**（模板 `$message(...)` 经 $mod 层 root 链回落解析，不再走 $sys 层；sandbox `$sys` 暴露面 sandbox.js:212 同步收窄）。sys.$message 挂载点删除，内核零 UI 依赖。回归 4000 全页面（$message 高频调用面）。
  **已落地（2026-08-28）**：①vhtml 内核删 src/vmessage.js + module.js createSystemContext 挂载点（dist 重建，vmsg 样式零残留，165 测试绿）；②vhtml-ui 新增 ui/vmessage.js（原样迁入）+ env.js `if (!all.globals.$message) all.define('$message', vmessage)` 守卫注册（防多挂载点重复 define 撞描述符；manager.clear() 后 globals 重建守卫自然放行）；③装配：aic/vbase/rses 各自根 env.js 加 `if (!all.globals.$message) await all.loadModule('v')`——aic 必须先于 loadModule("vb")（vb env 同守卫读，根先装 vb 才跳过自己的 /v，避免 /vb/v 404 噪音）；嵌套拓扑（rses→aic@v、vbase→aic）经守卫读收敛为单次装载。④vhtml docs 应用不挂 vhtml-ui（import 循环），examples/docs/runtime_smoke/preview 四页 $message 改用 console/window.* 内联；docs.html sys 表 + langs.json 文案标注「v0.10.3 起 vhtml-ui 提供」。**已回归（2026-08-29 用户重启 4000 验证：chat/explorer/usr 等 $message 高频面全正常）**。**文档归属（2026-08-29 用户定）**：vhtml SKILL.md 不承载 $message 知识（$sys 表行/`## $message` 章/description 提及/示例引用全部清除），说明迁入 vhtml-ui 自己的 ui/README.md「$message 全局消息」节；skill 镜像已同步。
- [x] 文档冻结评估：SKILL.md + skill 镜像 + README + CHANGELOG 终稿（2026-08-29，随本提交落定）。
- [x] 冻结落定后进入 semver 纪律：破坏性改动只走大版本（v0.11.x → 1.0.0）。（2026-08-29 起生效）
...
- **v-for 行模板计划化（AOT 行计划）**（2026-08-29 落地后同日撤回）：实测真实语料收益持平（纯结构行 -40% 无实际场景）；成本主体是绑定安装而非结构分发，计划化只省后者；instantiateRowPlan 对 compileVif/compileNode 的逐行镜像带来永久双份维护债，落地期间已产生 3 个漂移 bug。撤回清单与测量数据见 v0.10.3 AOT 条。
