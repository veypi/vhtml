 ...
 */
function mergeIntoProxy(oldValue, newValue) {
 ...
 */
function defineProperty(target, key, value, opts = {}) {
...
function createModuleContext(scoped, sharedLocale, initial = {}, broadcast = null, globals = null) {
  const mod = { ...initial };
  mod.scoped = scoped;
...
const RUNTIME = Symbol('vhtmlRuntime');
...
function hasProtocol$1(url) {
  return protocolPattern$1.test(url)
}
...
function parseUrlString(urlString, nav = null) {
  let url;
  let path = '';
  if (hasProtocol$1(urlString) && !isHttpUrl(urlString)) return null
...
function setCompileContext(ctx) {
...
function warnObserverFallback(node) {
 ...
 */
function getSharedTemplateNodes(key, extract) {
...
async function setupRef(dom, data, parentRuntime, target, instance, singleMode = false, ctx, ticket = null) {
